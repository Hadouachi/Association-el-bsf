'use client'

import Link from 'next/link'
import { Calendar, Newspaper, FileText, Settings, LogOut } from 'lucide-react'

interface AdminNavProps {
  locale: string
  currentPage?: 'dashboard' | 'activities' | 'news' | 'about' | 'settings'
}

export default function AdminNav({ locale, currentPage = 'dashboard' }: AdminNavProps) {
  const navItems = [
    {
      name: 'Tableau de bord',
      href: `/${locale}/admin`,
      icon: Settings,
      isActive: currentPage === 'dashboard'
    },
    {
      name: 'Actualités',
      href: `/${locale}/admin/news`,
      icon: Newspaper,
      isActive: currentPage === 'news'
    },
    {
      name: 'Activités',
      href: `/${locale}/admin/activities`,
      icon: Calendar,
      isActive: currentPage === 'activities'
    },
    {
      name: 'À propos',
      href: `/${locale}/admin/about`,
      icon: FileText,
      isActive: currentPage === 'about'
    }
  ]

  return (
    <div className="bg-white shadow-sm border-b">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-4">
          <div className="flex items-center space-x-8">
            <Link href={`/${locale}/admin`} className="text-xl font-bold text-gray-900">
              Administration
            </Link>
            <nav className="hidden md:flex space-x-6">
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className={`flex items-center space-x-2 px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                    item.isActive
                      ? 'bg-primary-100 text-primary-700'
                      : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100'
                  }`}
                >
                  <item.icon className="w-4 h-4" />
                  <span>{item.name}</span>
                </Link>
              ))}
            </nav>
          </div>
          
          <div className="flex items-center space-x-4">
            <span className="text-sm text-gray-500">Admin</span>
            <div className="w-8 h-8 bg-primary-600 rounded-full flex items-center justify-center">
              <span className="text-white text-sm font-medium">A</span>
            </div>
            <button
              onClick={() => {
                localStorage.removeItem('adminAuthenticated')
                window.location.href = `/${locale}/admin/login`
              }}
              className="flex items-center space-x-2 px-3 py-2 text-sm text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-md transition-colors"
              title="Se déconnecter"
            >
              <LogOut className="w-4 h-4" />
              <span className="hidden sm:inline">Déconnexion</span>
            </button>
          </div>
        </div>
      </div>
    </div>
  )
}
